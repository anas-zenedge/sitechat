...
public sealed class RagServiceTests
{
    ...
    public async Task ChatAsync_WithEmbeddedPages_ReturnsProviderAnswerAndSources()
    {
        ...
        var service = new RagService(repository.Object, openRouterClient.Object, options, NullLogger<RagService>.Instance);
        ...
        repository.Setup(repo => repo.SaveMessageAsync("session-1", "user", "Tell me about pricing", "site-1", null, It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        repository.Setup(repo => repo.GetSiteAsync("site-1", It.IsAny<CancellationToken>()))
        ...
        openRouterClient.Setup(client => client.CreateEmbeddingAsync(
            ...
            .ReturnsAsync([1d, 0d]);
        repository.Setup(repo => repo.GetPagesForRetrievalAsync("site-1", It.IsAny<CancellationToken>()))
            .ReturnsAsync([
        ...
        repository.Setup(repo => repo.SaveMessageAsync(
                "session-1",
                "assistant",
    ...
    public async Task ChatAsync_WhenChatCompletionFails_ReturnsFallbackAnswer()
    {
        ...
        repository.Setup(repo => repo.SaveMessageAsync("session-2", "user", "Where can I contact support?", null, null, It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        openRouterClient.Setup(client => client.CreateEmbeddingAsync(
                It.IsAny<EmbeddingRequest>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync([0.5d, 0.5d]);
        repository.Setup(repo => repo.GetPagesForRetrievalAsync(null, It.IsAny<CancellationToken>()))
